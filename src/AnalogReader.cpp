...
//====== Variable globale
volatile  byte __ADValue = 0;             // Valeur lue
...
void __callback_AnalogReader(void) {
  __ADValue = ADCH;          // Recuperation de la valeur mesurée par l'ADF
...
void  AnalogReader::begin(const byte pin) {
  ...
  ADMUX  |= (pin << MUX0); // choix de l'entree (0 a 7)
  ADMUX  |= (1 << REFS0); // reference voltage : Interne sur AVCC (0..3) 1=Vcc 3=1,1v
  ADMUX  |= (1 << ADLAR); //aligne ment a droite : mode 8 bit  (0/1)
